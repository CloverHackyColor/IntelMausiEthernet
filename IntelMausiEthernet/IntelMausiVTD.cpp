...
bool IntelMausi::setupRxMap()
{
    ...
    for (UInt32 i = 0, idx = 0; i < kNumRxMemDesc; i++, idx += kRxMemBatchSize) {
...
error_rx_desc:
    if (rxMapMem) {
        for (UInt32 i = 0; i < kNumRxMemDesc; i++) {
...
{
    if (txMapMem) {
        for (UInt32 i = 0; i < kNumTxMemDesc; i++) {
